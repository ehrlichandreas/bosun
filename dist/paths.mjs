 ...
 */
export function logRoot(from         = process.cwd())                {
  let dir = from;
  const { root } = parse(dir);
  for (;;) {
 ...
 */
export function unknownFlag(argv          , known          )                {
 ...
 */
export function sibling(metaUrl        , name        )         {
 ...
 */
export function alsoAbove(chosen        )           {
  const found           = [];
...
export function noLogRoot(from         = process.cwd())         {
 ...
 */
export function readEntryFile(path        )                          {
  let text        ;
  try {
    text = readFileSync(path, "utf8");
  } catch (error) {
    throw new Error(`${basename(path)} cannot be read: ${(error         ).message}`);
  }
  try {
    return JSON.parse(text)                           ;
  } catch (error) {
    throw new Error(
      `${basename(path)} is not valid JSON (${(error         ).message}). ` +
