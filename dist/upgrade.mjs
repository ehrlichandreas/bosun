...
import { LATEST, validate, versions } from "./schema.mjs";
import {
  logRoot,
  takeLog,
  unknownFlag,
...
export function deduplicate(entries                                  )           {
  ...
  for (const [id, group] of byId) {
    ...
    const ordered = [...group].sort((a, b) => {
      const digits = (one          ) =>
        Number(/^E(\d+)$/.exec((one.entry.alias          ) ?? "")?.[1] ?? 0);
...
export function walk(entry       , name        )                    {
...
function main(passed          )         {
  ...
  for (const name of names) {
    ...
    let entry       ;
    try {
      entry = readEntryFile(path)         ;
    } catch (error) {
      refused.push((error         ).message);
      continue;
    }
    const before = (entry.schema          ) ?? 1;
